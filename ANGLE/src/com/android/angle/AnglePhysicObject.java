...
public class AnglePhysicObject extends AngleObject {
	...
	// 新加点击触摸检测--lambp
	public boolean test(float x, float y, int wx, int hy) {
		// wx,hy分别为显示的图片的大小，不是图片的真实大小
		float minx = mPosition.mX - wx / 2;
		float miny = mPosition.mY - wx / 2;
		float maxx = mPosition.mX + hy / 2;
		float maxy = mPosition.mY + hy / 2;
