...
public class AngleTextureEngine
{
	...
	public AngleTexture createTextureFromResourceId(int resourceId)
	{
		...
		while (it.hasNext())
		{
			...
			{
				// Texture already exists
				if (((AngleResourceTexture) tex).mResourceID == resourceId && !((AngleResourceTexture) tex).oppose)
				{
					tex.mRefernces++;
					return tex;
				}
	...
	//建立翻转
	public AngleTexture createTextureFromResourceId(int resourceId,int mx,int my)
	{
		AngleTexture tex = null;
		Iterator<AngleTexture> it = mTexturesX.iterator();
		while (it.hasNext())
		...
		tex = new AngleResourceTexture(this, resourceId,mx,my);
		mTexturesX.add(tex);
		return tex;
	}
